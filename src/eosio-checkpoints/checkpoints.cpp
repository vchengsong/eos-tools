...
void blocklog::read_log() {
   ...
   auto print_info = [&](pbft_checkpoint_state& next) {
   ...
   auto checkpoints_dat = pbft_db_dir / "checkpoints.dat";
   if (fc::exists(checkpoints_dat)) {
      string content;
      fc::read_file_contents(checkpoints_dat, content);
      ...
      for (uint32_t i = 0, n = size.value; i < n; ++i) {
         ...
         fc::raw::unpack(ds, s);
         checkpoint_index.insert(std::make_shared<pbft_checkpoint_state>(move(s)));
      }
   } else {
      *out << "file not exist.\n";
   }
   ...
   for ( auto itr = by_blk_num.begin(); itr != by_blk_num.end(); itr++ ){
      pbft_checkpoint_state_ptr psp = *itr;
      print_info(*psp);
   }
}
